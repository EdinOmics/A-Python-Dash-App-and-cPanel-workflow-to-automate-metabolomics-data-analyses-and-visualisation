...
#TODO: Update for each individual user account
page_title_UserView = "Home Page: Alternative Example User" 
#TODO: Update each time you add a new project for this user
# (Should match the page_name specified in that page)
User_ListOfProjects = ("User Uploads Data Results Example")

#TODO: Use replace all to change the suffices of the input and output IDs
# from _ReportsAlternativeUserName to a unique name for this user's reports
...
layout = html.Div(
    [
        ...
        dcc.Location(id="url_ReportsAlternativeUserName"),              # fires once on page load
        # html.Div(id="page-content"),         # placeholder
        dbc.Spinner(
            children=[
                html.Div(id="page-content_ReportsAlternativeUserName"),         # placeholder
                ],
            size = "lg", 
            color = "primary", 
            fullscreen = True
...
@callback(
    Output("page-content_ReportsAlternativeUserName", "children"),
    Input("url_ReportsAlternativeUserName", "pathname"),               # fires on initial page load
)
